...
radioButtons("Area","Which location level data to display?",c("ECC Zones" = "z", "Census Tracts" = "c"),selected="z")
```
...
filteredData_variable <- reactive({
  ...
  #adataset <- atlasdata %>%
   ...
   adataset2 <- atlasdata2 %>% 
   left_join(metadata_popup) %>% 
  filter(description == variable() & type == input$pct_count)
  suppressWarnings(left_join(es_zones, atlasdata2, by = "School") %>% 
                     mutate(moe_disp=if_else(moe==-99,"NA",as.character(moe)),
                      popup_text=paste("<b>Census Tract: </b>", 
                                       #substr(GEOID,6,11),
                                       "<br>",
